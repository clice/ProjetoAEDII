...
// Estrutura de um nó do MaxHeap
typedef struct {
    int *data; // O dado (um ponteiro para um número inteiro neste caso)
    int priority; // A prioridade (o valor inteiro associado ao ponteiro)
} MaxHeapNode;

// Estrutura do MaxHeap
typedef struct {
    MaxHeapNode **array; // Array de ponteiros para os nós do heap
    int capacity; // Capacidade máxima do heap
    int size; // Tamanho atual do heap
} MaxHeap;

// Função para criar um novo nó do MaxHeap
...
// Função para criar um novo MaxHeap
...
// Função para ajustar o MaxHeap após a inserção de um novo nó
...
// Função para inserir um novo nó no MaxHeap
...
// Função para extrair o nó com maior prioridade (raiz) do MaxHeap
...
// Função para imprimir os elementos do MaxHeap
